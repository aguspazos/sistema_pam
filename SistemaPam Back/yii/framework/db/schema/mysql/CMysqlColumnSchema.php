...
class CMysqlColumnSchema extends CDbColumnSchema
{
	...
	protected function extractType($dbType)
	{
		if(strncmp($dbType,'enum',4)===0)
			$this->type='string';
		else if(strpos($dbType,'datetime')!==false || strpos($dbType,'date')!==false)
			$this->type='date';
		else if(strpos($dbType,'float')!==false || strpos($dbType,'double')!==false)
			$this->type='double';
		else if(strpos($dbType,'bool')!==false || strpos($dbType,'tinyint(1)')!==false)
			$this->type='boolean';
		else if(strpos($dbType,'int')===0 && strpos($dbType,'unsigned')===false || preg_match('/(bit|smallint|mediumint)/',$dbType))
			$this->type='integer';
		else
			$this->type='string';
	}
