...
class AdministratorsController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            array(
                'allow',
                'actions' => array('viewLogin', 'login','add'),
                'users' => array('*'),
            ),
            array(
                'allow',
                ...
                if (isset($_POST['email']) && isset($_POST['password'])) {
                    ...
                    if (count($consecutiveFailedLoginAttemptsInLastHour) < 5) {
                        if ($model->validate() && $model->loginAdmin()) {
                            $administrator = Administrators::getByEmail($_POST['email']);
                            $administrator->refreshToken();
                            $response['status'] = 'ok';
                            $response['token'] = $administrator->token;
                            $response['role'] = $administrator->administrator_role_id;
    ...
    public function actionAdd()
    {
        $response = array();
        try {
            if ($this->administrator && $this->administrator->administrator_role_id == AdministratorRoles::$SUPER_ADMIN) {
                if (isset($_POST['email']) && isset($_POST['name']) && isset($_POST['last_name']) && isset($_POST['phone']) && isset($_POST['administrator_role_id'])  && isset($_POST['password'])) {
                    if (Administrators::checkUnique($_POST['email'])) {
                        $administrator = Administrators::create($_POST['email'],$_POST['name'],$_POST['last_name'],$_POST['phone'],$_POST['administrator_role_id'],1,$_POST['password']);
