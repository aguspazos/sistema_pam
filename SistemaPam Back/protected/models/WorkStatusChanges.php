...
class WorkStatusChanges extends CActiveRecord
{
    ...
    public static function getAttributeName($name)
    {
        switch ($name) {
            ...
            case 'work_id':
                return 'WorkId';
            case 'original_work_status_id':
                return 'OriginalWorkStatusId';
            case 'final_work_status_id':
                return 'FinalWorkStatusId';
            case 'notes':
                return 'Notes';
            case 'created_on':
                return 'CreatedOn';
    ...
    public static function getAllFromWorkAndFinalStatus($workId, $workFinalStatusId)
    {
        return self::model()->findAll(
            'id>0 AND deleted=0 AND work_id = :workId and final_work_status_id = :finalStatus',
            array(
                'workId' => $workId,
                'finalStatus' => $workFinalStatusId
            )
        );
    }
